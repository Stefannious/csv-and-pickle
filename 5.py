    ...
    with open(f'{name_table}.csv', 'r', encoding='utf-8') as f:
        ...
        for colum, typee in dict_types.items():
            ...
            if typee not in table[0][now_colum][-5:]:
                ...
                for line in table[1:]:
                    if typee == 'str':
                        line[now_colum] = str(line[now_colum])
                    elif typee == 'int':
                        line[now_colum] = int(line[now_colum])
                    elif typee == 'float':
                        line[now_colum] = float(line[now_colum])
                    elif typee == 'bool':
                        line[now_colum] = bool(line[now_colum])
...
def load_table(name_table):
    with open(f'{name_table}.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        while True:
            my_request = input('Набор команд:выйти с таблицы, вывести таблицу, получание по строкам, получить типы, задать типы, получить по индексам, дабавить значения, обьединить таблицы, разбить на две\n').lower()
            if my_request == 'выйти с таблицы': break
            ...
            if my_request == 'вывести таблицу':
                ...
                new_request = dict(
                    input('Введите ключ(номер столбца) и значение через пробел по одному\n').split(' ') for _ in
                    range(2))
            ...
            elif my_request == 'дабавить значения':
...
def save_table(table, name_table):  # ввод таблицки в видде список-в-списке - проверенно, работает
    with open(f'{name_table}.csv', 'w', newline='',
              encoding='utf-8') as f:  # сохранить Name, Sur Name, Fathername!Alex, Serov, Dmitrievich! Egor, Serov, Dmitrievich
        writer = csv.writer(f)
